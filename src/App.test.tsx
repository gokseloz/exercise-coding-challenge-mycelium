...
describe("Catalog Viewer", () => {
  ...
  let toggleSlideShowBtn: HTMLInputElement;
  let catalogs: any;
  let thumbBtn2: HTMLButtonElement;
  let thumbBtn4: HTMLButtonElement;

  beforeEach(() => {
    render(<App />);
    viewer = screen.getByRole("img", { name: /catalog\-view/i });
  ...
  test("clicking on the previous button should show the previous image", () => {
    fireEvent.click(thumbBtn2, { button: "0" });
    expect(viewer.src).toContain(catalogs[1].image);
  ...
  test("clicking on the next button should show the next image", () => {
    fireEvent.click(thumbBtn2, { button: "0" });
    fireEvent.click(prevBtn, { button: "0" });
    expect(viewer.src).toContain(catalogs[0].image);
